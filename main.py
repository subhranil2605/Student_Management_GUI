...
def next_page_1(frame):
    ...
    if (
        ...
        messagebox.showerror(
            "Error", "Please Enter the Percentage of Disability.", parent=root
        )
    elif dob == tdy:
        messagebox.showerror(
            "Date Entry Error", "Please Select Date of Birth.", parent=root
        )
    elif student_id == "":
        value = messagebox.askquestion(
            "Students Id.",
            "Want to go to the Next Page without Student ID?",
            parent=root,
        )
...
def upload_image():
    ...
    if filename == "":
        ...
        try:
            ...
            cur.execute(
                f"INSERT INTO {ses} VALUES(?, ?)", (ent_email.get(), image_data)
            )
            conn.commit()
            ...
            messagebox.showerror(
                "Upload Error", "Please select correct image.", parent=root
            )
...
def sign_in(frame):
    ...
    if session_selected == "Select" and regno_si == "" and email_si == "":
        ...
        try:
            cur.execute(
                f"SELECT * FROM  {ses_select}  WHERE regNo = ? AND email = ?",
                (regno_si, email_si),
            )
            ...
            if personal_data == None:
                ...
                cur.execute(
                    f"SELECT * FROM {ses_select} where email = (?)",
                    (ent_email_si.get(),),
                )
                row = cur.fetchone()
                conn.commit()
        ...
        # except sqlite3.OperationalError:
            ...
            messagebox.showerror(
                "Error", f"Session {es} Does Not Exist!!!", parent=root
            )
...
def get_cursor(ev):
    ...
    real_row = student_table.identify_row(
        student_table.winfo_pointerxy()[1] - student_table.winfo_rooty()
    )
    if row != "":
        btn_delete_student.config(state=NORMAL)
        btn_show_student.config(state=NORMAL)
...
def delete_data_from_database():
    ...
    if email_selected == "":
        ...
        btn_show_student.config(state=DISABLED)
        messagebox.showerror(
            "Error", "Please select a student from table!!!", parent=root
        )
    else:
        q = messagebox.askquestion(
            "Delete",
            f"Do you want to delete the student {ent_selected_name.get()}?",
            parent=root,
        )
        if q == "yes":
            cur.execute(
                f"DELETE FROM {ses_select} WHERE email = (?)", (email_selected,)
            )
            conn.commit()
...
def show_details(frame):
    ...
    if email_selected == "":
        ...
        cur = conn.cursor()
        cur.execute(
            f"SELECT * FROM {ses_select} where email = (?)", (ent_selected_email.get(),)
        )
...
lbl_college_details_name = Label(
    frame_header_homepage,
    text="Krishnagar Government College",
    font=("times", 25, "bold"),
    bg="#F0FFFF",
)
lbl_college_details_name.place(
    x=50,
    y=10,
)

lbl_college_details = Label(
    frame_header_homepage,
...
btn_admin_login = Button(
    ...
    bd=2,
    relief=RIDGE,
    command=lambda: swap(frame_admin_homepage),
)
btn_admin_login.pack(side=LEFT)

# Button for home
btn_admin_home = Button(
    frame_black_homeapage,
    text="Home",
    bg="black",
    fg="white",
    ...
    bd=2,
    relief=RIDGE,
    command=lambda: swap(frame_department),
)
...
lbl_greeting = Label(
    frame_department,
    text="Welcome to Krishnagar Govt. College",
    fg="#3B444B",
    bg="#F0F8FF",
...
lbl_department_name_homepage = Label(
    frame_department,
...
btn_students_zone = Button(
    ...
    command=lambda: swap(frame_student_zone),
)
...
btn_students_zone = Button(
    ...
    bd=1,
    relief=RAISED,
    activebackground="#301934",
    activeforeground="white",
    command=lambda: register(frame_register),
)
...
btn_students_zone = Button(
    ...
    activebackground="#301934",
    activeforeground="white",
    command=lambda: swap(frame_login_screen),
)
...
# Label sign in
label_sign_in = Label(
    frame_login_screen, text="Log In", bg="#F0F8FF", font=("meera", 22)
)
label_sign_in.pack(pady=20)


# selecting session
lbl_session_si = Label(
    master=frame_login_screen, text="Session", font=("times", 13), bg="#F0F8FF"
)
...
lbl_registration_si = Label(
    frame_login_screen, text="Registration Number", bg="#F0F8FF", font=("times", 13)
)
lbl_registration_si.place(x=75, y=150)

ent_registration_si = Entry(
    frame_login_screen, font=("times", 15), bg="whitesmoke", relief=SOLID, bd=1
)
...
# Sign In email
lbl_email_si = Label(
    frame_login_screen, text="Email Address", bg="#F0F8FF", font=("times", 13)
)
...
btn_sign_in = Button(
    ...
    activeforeground="white",
    relief=FLAT,
    command=lambda: sign_in(student_personal_homepage_2),
)
...
bg_bdy = "#90caf9"
# Name
Label(master=tab1_2, text="Full Name", font=("Comic Sans MS", 13), bg=bg_bdy).place(
    x=10, y=10
)
ent_name_get_2 = Entry(
    master=tab1_2,
    bg="white",
    font=("times", 13),
    bd=1,
...
# Sex
Label(master=tab1_2, text="Sex", font=("Comic Sans MS", 13), bg=bg_bdy).place(
    x=10, y=80, height=30
)
lbl_sex_get_2 = Label(
    tab1_2, text="", font=("times", 13), bg="white", bd=1, relief=SOLID
)
lbl_sex_get_2.place(x=210, y=80, height=30, width=150)

# Category
Label(master=tab1_2, text="Category", font=("Comic Sans MS", 13), bg=bg_bdy).place(
    x=10, y=120
)
lbl_cat_get_2 = Label(
    tab1_2, text="", font=("times", 13), bg="white", bd=1, relief=SOLID
...
# Religion
Label(master=tab1_2, text="Religion", font=("Comic Sans MS", 13), bg=bg_bdy).place(
    x=10, y=160
)
lbl_religion_get_2 = Label(
    tab1_2, text="", font=("times", 13), bg="white", bd=1, relief=SOLID
)
...
# Nationality
Label(master=tab1_2, text="Nationality", font=("Comic Sans MS", 13), bg=bg_bdy).place(
    x=10, y=200
)
lbl_nationality_get_2 = Label(
    tab1_2, text="", font=("times", 13), bg="white", bd=1, relief=SOLID
)
...
Label(
    master=tab1_2, text="Physically Handicapped?", font=("Comic Sans MS", 13), bg=bg_bdy
).place(x=10, y=240)
lbl_physical_handicap_get_2 = Label(
    tab1_2, text="", font=("times", 13), bg="white", bd=1, relief=SOLID
)
...
# % of Disability
Label(
    master=tab1_2, text="% of Disability", font=("Comic Sans MS", 13), bg=bg_bdy
).place(x=10, y=280)
lbl_percntg_hc_get_2 = Label(
    tab1_2, text="", font=("times", 13), bg="white", bd=1, relief=SOLID
)
...
# 	Date of Birth
Label(master=tab1_2, text="Date of Birth", font=("Comic Sans MS", 13), bg=bg_bdy).place(
    x=10, y=320
)
lbl_dob_get_2 = Label(
    tab1_2, text="", font=("times", 13), bg="white", bd=1, relief=SOLID
...
# Country Name
Label(master=tab1_2, text="Country", font=("Comic Sans MS", 13), bg=bg_bdy).place(
    x=10, y=360
)
lbl_country_get_2 = Label(
    tab1_2, text="", font=("times", 13), bg="white", bd=1, relief=SOLID
)
...
# Aadhaar No.
Label(
    master=tab1_2, text="Aadhaar Number", font=("Comic Sans MS", 13), bg=bg_bdy
).place(x=10, y=400)

ent_aadhaar_get_2 = Entry(
    master=tab1_2,
    bg="white",
    font=("times", 13),
    bd=1,
...
# ----------------------1st tab 2nd column---------------------------------------------
# Father's Name
Label(master=tab1_2, text="Father's Name", font=("Comic Sans MS", 13), bg=bg_bdy).place(
    x=400, y=10
)

ent_father_name_get_2 = Entry(
    master=tab1_2,
    bg="white",
    font=("times", 13),
    bd=1,
...
# 	Father's Occupation
Label(
    master=tab1_2, text="Father's Occupation", font=("Comic Sans MS", 13), bg=bg_bdy
).place(x=400, y=80)

ent_father_occu_get_2 = Entry(
    master=tab1_2,
    bg="white",
    font=("times", 13),
    bd=1,
...
# Mother's Name
Label(master=tab1_2, text="Mother's Name", font=("Comic Sans MS", 13), bg=bg_bdy).place(
    x=400, y=150
)

ent_mother_name_get_2 = Entry(
    master=tab1_2,
    bg="white",
    font=("times", 13),
    bd=1,
...
# 	Mother's Occupation
Label(
    master=tab1_2, text="Mother's Occupation", font=("Comic Sans MS", 13), bg=bg_bdy
).place(x=400, y=220)

ent_mother_occu_get_2 = Entry(
    master=tab1_2,
    bg="white",
    font=("times", 13),
    bd=1,
...
# Guardian Name
Label(master=tab1_2, text="Guardian Name", font=("Comic Sans MS", 13), bg=bg_bdy).place(
    x=400, y=290
)

ent_guardian_get_2 = Entry(
    master=tab1_2,
    bg="white",
    font=("times", 13),
    bd=1,
...
# Guardian Relation
Label(
    master=tab1_2, text="Relation with Guardian", font=("Comic Sans MS", 13), bg=bg_bdy
).place(x=400, y=360)

ent_guardian_rel_get_2 = Entry(
    master=tab1_2,
    bg="white",
    font=("times", 13),
    bd=1,
...
Label(
    master=tab1_2,
    text="Guardian's Email Address",
    font=("Comic Sans MS", 13),
    bg=bg_bdy,
).place(x=400, y=430)

ent_g_email_get_2 = Entry(
    master=tab1_2,
    bg="white",
    font=("times", 13),
    bd=1,
...
# Session
Label(master=tab1_2, text="Session", font=("Comic Sans MS", 13), bg=bg_bdy).place(
    x=760, y=10
)
lbl_session_get_2 = Label(
    tab1_2, text="", font=("times", 13), bg="white", bd=1, relief=SOLID
)
lbl_session_get_2.place(x=860, y=10, height=30, width=120)

# Registration number
Label(master=tab1_2, text="Reg. No.", font=("Comic Sans MS", 13), bg=bg_bdy).place(
    x=760, y=50
)

ent_registration_get_2 = Entry(
    master=tab1_2,
    bg="white",
    font=("times", 13),
    bd=1,
...
# Student Id
Label(master=tab1_2, text="Student Id", font=("Comic Sans MS", 13), bg=bg_bdy).place(
    x=760, y=120
)

ent_student_id_get_2 = Entry(
    master=tab1_2,
    bg="white",
    font=("times", 13),
    bd=1,
...
# Course
Label(master=tab1_2, text="Course", font=("Comic Sans MS", 13), bg=bg_bdy).place(
    x=760, y=190
)
lbl_get_course_2 = Label(
    tab1_2, text="", font=("times", 13), bg="white", bd=1, relief=SOLID
)
...
# Phone Number
Label(
    master=tab1_2, text="Contact Number", font=("Comic Sans MS", 13), bg=bg_bdy
).place(x=760, y=230)

ent_phone_get_2 = Entry(
    master=tab1_2,
    bg="white",
    font=("times", 13),
    bd=1,
...
# Email Address
Label(master=tab1_2, text="Email Address", font=("Comic Sans MS", 13), bg=bg_bdy).place(
    x=760, y=300
)

ent_email_get_2 = Entry(
    master=tab1_2,
    bg="white",
    font=("times", 13),
    bd=1,
...
Label(
    master=tab1_2,
    text="Guardian's Contact Number",
    font=("Comic Sans MS", 13),
    bg=bg_bdy,
).place(x=760, y=370)

ent_g_phone_get_2 = Entry(
    master=tab1_2,
    bg="white",
    font=("times", 13),
    bd=1,
...
btn_home_tab1_2 = Button(
    ...
    activebackground="green",
    activeforeground="white",
    command=lambda: swap(main_frame_homepage),
)
...
# Permanent Address
Label(
    master=tab2_2, text="Permanent Address", font=("Comic Sans MS", 13), bg=bg_bdy
).place(x=10, y=10)

text_prmnt_add_get_2 = Text(
    master=tab2_2,
    bg="white",
    font=("times", 12),
    bd=1,
...
# Pin Code
Label(master=tab2_2, text="Pincode", font=("Comic Sans MS", 13), bg=bg_bdy).place(
    x=10, y=150
)

ent_pincode_p_get_2 = Entry(
    master=tab2_2,
    bg="white",
    font=("times", 13),
    bd=1,
...
# Present Address
Label(
    master=tab2_2, text="Present Address", font=("Comic Sans MS", 13), bg=bg_bdy
).place(x=10, y=220)

text_prsnt_add_get_2 = Text(
    master=tab2_2,
    bg="white",
    font=("times", 12),
    bd=1,
...
# --------------------2nd Tab 2nd column---------------------------
# City name
Label(master=tab2_2, text="City", font=("Comic Sans MS", 13), bg=bg_bdy).place(
    x=360, y=10
)

ent_city_get_2 = Entry(
    master=tab2_2,
    bg="white",
    font=("times", 13),
    bd=1,
...
# District Name
Label(master=tab2_2, text="District", font=("Comic Sans MS", 13), bg=bg_bdy).place(
    x=360, y=80
)

ent_district_get_2 = Entry(
    master=tab2_2,
    bg="white",
    font=("times", 13),
    bd=1,
...
# State Name
Label(master=tab2_2, text="State", font=("Comic Sans MS", 13), bg=bg_bdy).place(
    x=360, y=150
)

ent_state_get_2 = Entry(
    master=tab2_2,
    bg="white",
    font=("times", 13),
    bd=1,
...
# Last Institution
Label(
    master=tab2_2, text="Last Institution Name", font=("Comic Sans MS", 13), bg=bg_bdy
).place(x=700, y=10)

ent_last_instn_get_2 = Entry(
    master=tab2_2,
    bg="white",
    font=("times", 13),
    bd=1,
...
Label(
    master=tab2_2,
    text="Last Institution Address",
    font=("Comic Sans MS", 13),
    bg=bg_bdy,
).place(x=700, y=80)

txt_last_instn_addrs_get_2 = Text(
    master=tab2_2, bg="white", font=("times", 12), bd=1, relief=SOLID, cursor="arrow"
)
txt_last_instn_addrs_get_2.place(x=700, y=110, width=360, height=60)


# Hobbies
Label(master=tab2_2, text="Hobbies", font=("Comic Sans MS", 13), bg=bg_bdy).place(
    x=700, y=180
)

txt_hobby_get_2 = Text(
    master=tab2_2, font=("times", 13), bg="white", bd=1, relief=SOLID, cursor="arrow"
)
txt_hobby_get_2.place(x=700, y=210, width=360, height=60)


# Frame for marks
frame_marks_get_2 = Frame(
    tab2_2,
    bg="lightgray",
    bd=1,
    relief=SOLID,
...
# Serial Number
Label(
    frame_marks_get_2, text="Sl.\nNo.", font=("times", 13, "bold"), bd=1, relief=RAISED
).place(x=0, y=0, width=40, height=50)

# Exam name
Label(
    frame_marks_get_2, text="Exam Name", font=("times", 13, "bold"), bd=1, relief=RAISED
).place(x=40, y=0, width=170, height=50)
...
Label(
    ...
    bd=1,
    relief=RAISED,
).place(x=210, y=0, width=200, height=50)
...
Label(
    ...
    bd=1,
    relief=RAISED,
).place(x=410, y=0, width=70, height=50)
...
Label(
    ...
    bd=1,
    relief=RAISED,
).place(x=480, y=0, width=70, height=50)

# Percentage
Label(
    frame_marks_get_2, text="%", font=("times", 13, "bold"), bd=1, relief=RAISED
).place(x=550, y=0, width=70, height=50)

# CGPA
Label(
    frame_marks_get_2, text="CGPA", font=("times", 13, "bold"), bd=1, relief=RAISED
).place(x=620, y=0, width=99, height=50)

# -----------------------------------Data--------------------------------------------

# Serial Numbers
Label(
    frame_marks_get_2, text="1", font=("times", 13, "bold"), bd=1, relief=SOLID
).place(x=-1, y=49, height=30, width=41)
Label(
    frame_marks_get_2, text="2", font=("times", 13, "bold"), bd=1, relief=SOLID
).place(x=-1, y=78, height=30, width=41)
Label(
    frame_marks_get_2, text="3", font=("times", 13, "bold"), bd=1, relief=SOLID
).place(x=-1, y=107, height=30, width=41)
Label(
    frame_marks_get_2, text="4", font=("times", 13, "bold"), bd=1, relief=SOLID
).place(x=-1, y=136, height=30, width=41)


# Exam Names

ent_exam1_get_2 = Entry(
    frame_marks_get_2,
    bg="white",
    font=("times", 13),
    bd=1,
...
lbl_title = Label(
    master=frame_body,
    text="Register as a New Student",
    bg=bg_bdy,
    fg="red",
...
# Name
lbl_name = Label(
    master=frame_body, text="Full Name", font=("Comic Sans MS", 13), bg=bg_bdy
)
lbl_name.place(x=50, y=70)

ent_name = Entry(
    master=frame_body, bg="whitesmoke", font=("times", 13), bd=1, relief=SOLID
)
...
cmb_sex = ttk.Combobox(
    master=frame_body,
    values=("Select", "Male", "Female", "TG"),
    font=("times", 13),
    state="readonly",
    justify=CENTER,
)
...
# Category
lbl_category = Label(
    master=frame_body, text="Category", font=("Comic Sans MS", 13), bg=bg_bdy
)
lbl_category.place(x=50, y=190)

cmb_category = ttk.Combobox(
    master=frame_body,
    values=("Select", "GENERAL", "SC", "ST", "OBC-A", "OBC-B"),
    font=("times", 13),
    state="readonly",
    justify=CENTER,
)
...
# Religion
lbl_religion = Label(
    master=frame_body, text="Religion", font=("Comic Sans MS", 13), bg=bg_bdy
)
lbl_religion.place(x=50, y=235)

cmb_religion = ttk.Combobox(
    master=frame_body,
    values=("Select", "Christianity", "Hinduism", "Islam", "Humanity", "Others"),
    font=("times", 13),
    state="readonly",
    justify=CENTER,
)
...
lbl_nationality = Label(
    master=frame_body, text="Nationality", font=("Comic Sans MS", 13), bg=bg_bdy
)
lbl_nationality.place(x=50, y=280)

cmb_nationality = ttk.Combobox(
    master=frame_body,
    values=("Select", "Indian", "Others"),
    font=("times", 13),
    state="readonly",
    justify=CENTER,
)
...
# 	% of Disability
lbl_per_ph = Label(
    master=frame_body, text="% of Disability", font=("Comic Sans MS", 13), bg=bg_bdy
)
...
# 	Date of Birth
lbl_dob = Label(
    master=frame_body, text="Date of Birth", font=("Comic Sans MS", 13), bg=bg_bdy
)
...
# Father's Name
lbl_father_name = Label(
    master=frame_body, text="Father's Name", font=("Comic Sans MS", 13), bg=bg_bdy
)
lbl_father_name.place(x=450, y=115)

ent_father_name = Entry(
    master=frame_body, bg="whitesmoke", font=("times", 13), bd=1, relief=SOLID
)
ent_father_name.place(x=450, y=145, height=30, width=320)

# 	Father's Occupation
lbl_father_occu = Label(
    master=frame_body, text="Father's Occupation", font=("Comic Sans MS", 13), bg=bg_bdy
)
lbl_father_occu.place(x=450, y=190)

ent_father_occu = Entry(
    master=frame_body, bg="whitesmoke", font=("times", 13), bd=1, relief=SOLID
)
ent_father_occu.place(x=450, y=220, height=30, width=320)

# Mother's Name
lbl_mother_name = Label(
    master=frame_body, text="Mother's Name", font=("Comic Sans MS", 13), bg=bg_bdy
)
lbl_mother_name.place(x=450, y=265)

ent_mother_name = Entry(
    master=frame_body, bg="whitesmoke", font=("times", 13), bd=1, relief=SOLID
)
ent_mother_name.place(x=450, y=295, height=30, width=320)

# 	Mother's Occupation
lbl_mother_occu = Label(
    master=frame_body, text="Mother's Occupation", font=("Comic Sans MS", 13), bg=bg_bdy
)
lbl_mother_occu.place(x=450, y=340)

ent_mother_occu = Entry(
    master=frame_body, bg="whitesmoke", font=("times", 13), bd=1, relief=SOLID
)
...
# Guardian Name
lbl_guardian = Label(
    master=frame_body, text="Guardian Name", font=("Comic Sans MS", 13), bg=bg_bdy
)
lbl_guardian.place(x=820, y=70)

ent_guardian = Entry(
    master=frame_body, font=("times", 13), bg="whitesmoke", bd=1, relief=SOLID
)
ent_guardian.place(x=820, y=100, height=30, width=330)


# Guardian Relation
lbl_guardian_rel = Label(
    master=frame_body,
...
# Session
lbl_session = Label(
    master=frame_body, text="Session", font=("Comic Sans MS", 13), bg=bg_bdy
)
...
# Registration Number
lbl_registrarion = Label(
    master=frame_body, text="Reg. No.", font=("Comic Sans MS", 13), bg=bg_bdy
)
lbl_registrarion.place(x=820, y=265)

ent_registration = Entry(
    master=frame_body, font=("times", 13), bg=bg_bdy, bd=1, relief=SOLID
)
ent_registration.place(x=950, y=265, width=200, height=30)


# Student Id
lbl_student_id = Label(
    master=frame_body, text="Student Id", font=("Comic Sans MS", 13), bg=bg_bdy
)
lbl_student_id.place(x=820, y=310)

ent_student_id = Entry(
    master=frame_body, font=("times", 13), bg=bg_bdy, bd=1, relief=SOLID
)
...
# Course
lbl_course = Label(
    master=frame_body, text="Course", font=("Comic Sans MS", 13), bg=bg_bdy
)
lbl_course.place(x=820, y=355)

cmb_course = ttk.Combobox(
    master=frame_body,
    values=("Select", "HONOURS", "GENERAL"),
    font=("times", 13),
    state="readonly",
    justify=CENTER,
)
...
btn_next_1 = Button(
    ...
    bg="green",
    fg="white",
    command=lambda: next_page_1(frame2_body),
)
...
# Phone Number
lbl_phone = Label(
    master=frame2_body, text="Contact Number *", font=("Comic Sans MS", 13), bg=bg_bdy
)
lbl_phone.place(x=50, y=30)

ent_phone = Entry(
    master=frame2_body, font=("times", 13), bg="whitesmoke", bd=1, relief=SOLID
)
ent_phone.place(x=50, y=60, height=30, width=300)

# Email Address
lbl_email = Label(
    master=frame2_body, text="Email Address *", font=("Comic Sans MS", 13), bg=bg_bdy
)
...
# Aadhaar No.
lbl_aadhaar = Label(
    master=frame2_body, text="Aadhaar Number *", font=("Comic Sans MS", 13), bg=bg_bdy
)
...
text_prmnt_add = Text(
    master=frame2_body, bg="whitesmoke", font=("times", 12), bd=1, relief=SOLID
)
text_prmnt_add.place(x=400, y=60, width=330, height=60)

# Pin Code
lbl_pincode_p = Label(
    master=frame2_body, text="Pincode *", font=("Comic Sans MS", 13), bg=bg_bdy
)
lbl_pincode_p.place(x=400, y=140)

ent_pincode_p = Entry(
    master=frame2_body, bg="whitesmoke", font=("times", 13), bd=1, relief=SOLID
)
ent_pincode_p.place(x=500, y=140, height=30, width=230)


# Checking if the two address are same
lbl_same_address = Label(
    master=frame2_body,
    text="Is your Present Address same as Permanent Address?",
    font=("times", 12),
    bg=bg_bdy,
    fg="blue",
)
...
# Present Address
lbl_prsnt_add = Label(
    master=frame2_body, text="Present Address *", font=("Comic Sans MS", 13), bg=bg_bdy
)
...
# City name
lbl_city = Label(
    master=frame2_body, text="City *", font=("Comic Sans MS", 13), bg=bg_bdy
)
...
# District Name
lbl_district = Label(
    master=frame2_body, text="District *", font=("Comic Sans MS", 13), bg=bg_bdy
)
...
# State Name
lbl_state = Label(
    master=frame2_body, text="State *", font=("Comic Sans MS", 13), bg=bg_bdy
)
...
# Country Name
lbl_country = Label(
    master=frame2_body, text="Country", font=("Comic Sans MS", 13), bg=bg_bdy
)
...
btn_previous_1 = Button(
    master=frame2_body,
    text="Previous Page",
    font=("Comic Sans MS", 13),
    bg="green",
    fg="white",
    command=lambda: swap(frame_body),
)
...
btn_next_2 = Button(
    ...
    bg="green",
    fg="white",
    command=lambda: next_page_2(frame3_body),
)
...
ent_last_instn = Entry(
    master=frame3_body, font=("times", 13), bg="whitesmoke", bd=1, relief=SOLID
)
ent_last_instn.place(x=50, y=60, height=30, width=330)

# Last Institution Address
lbl_last_instn_addrs = Label(
    master=frame3_body,
    text="Last Institution Address *",
    font=("Comic Sans MS", 13),
    bg=bg_bdy,
...
txt_last_instn_addrs = Text(
    master=frame3_body, bg="whitesmoke", font=("times", 12), bd=1, relief=SOLID
)
txt_last_instn_addrs.place(x=50, y=135, width=330, height=60)

# Hobbies
lbl_hobby = Label(
    master=frame3_body, text="Hobbies *", font=("Comic Sans MS", 13), bg=bg_bdy
)
...
# Exam name
lbl_exam_name = Label(
    frame_marks, text="Exam Name", font=("times", 13, "bold"), bd=1, relief=RAISED
)
lbl_exam_name.place(x=40, y=0, width=170, height=50)

# Board name
lbl_board_name = Label(
    frame_marks, text="Board Name", font=("times", 13, "bold"), bd=1, relief=RAISED
)
lbl_board_name.place(x=210, y=0, width=200, height=50)

# passing year
lbl_passing_year = Label(
    frame_marks, text="Passing\nyear", font=("times", 13, "bold"), bd=1, relief=RAISED
)
lbl_passing_year.place(x=410, y=0, width=70, height=50)

# Marks
lbl_marks = Label(
    frame_marks, text="Total\nMarks", font=("times", 13, "bold"), bd=1, relief=RAISED
)
lbl_marks.place(x=480, y=0, width=70, height=50)


# Percentage
lbl_percentage = Label(
    frame_marks, text="%", font=("times", 13, "bold"), bd=1, relief=RAISED
)
...
# CGPA
lbl_cgpa = Label(
    frame_marks, text="CGPA", font=("times", 13, "bold"), bd=1, relief=RAISED
)
...
# Previous page button
btn_previous_2 = Button(
    master=frame3_body,
    text="Previous Page",
    font=("Comic Sans MS", 13),
    bg="green",
    fg="white",
    command=lambda: swap(frame2_body),
)
...
Label(
    frame_status_bar,
    text="Developed by Subhranil Sarkar",
    fg="black",
    bg="whitesmoke",
).pack(side=RIGHT)
...
frame_status_bar_homepage = Frame(
    main_frame_homepage, bd=2, relief=SUNKEN, bg="whitesmoke"
)
...
label_password_admin = Label(
    frame_admin_password, text="Password", bg="white", font=("meera", 25)
)
label_password_admin.pack(pady=50)

ent_admin_password = Entry(
    frame_admin_password, font=("meera", 18), bg="whitesmoke", show="*"
)
ent_admin_password.place(x=70, y=130, width=250, height=30)

btn_admin_pass_login = Button(
    ...
    activeforeground="white",
    command=lambda: check_pass(frame_admin_panel),
)
btn_admin_pass_login.place(x=195, y=200, width=100, height=50)

btn_back_homepage = Button(
    frame_admin_password,
    text="Back",
    bg="#CC0000",
    fg="white",
...
btn_backtohome = Button(
    frame_admin_panel,
    text="Home",
    font=("times", 15),
    fg="white",
    bg="black",
    command=lambda: gotohome(main_frame_homepage),
)
...
student_table.heading(
    "guardian_contact", text="Guardian Contact Number"
)  # guardian no.
...
student_table.heading(
    "last_instn_add", text="Last Institution Address"
)  # last instn address
...
# title
Label(
    frame_admin_control, text="Selected Student", font=("times", 20), bg="white"
).place(x=10, y=10, height=40)

# Entry field
lbl_selected_name = Label(frame_admin_control, text="Name", bg="white")
lbl_selected_name.place(x=10, y=50, width=50)

ent_selected_name = Entry(
    frame_admin_control, bg="lightgray", state=DISABLED, disabledforeground="red"
)
...
btn_delete_student = Button(
    ...
    bg="black",
    state=DISABLED,
    command=delete_data_from_database,
)
btn_delete_student.place(x=500, y=100, width=60, height=30)

# show button
btn_show_student = Button(
    frame_admin_control,
    text="Show Details",
    font=("times", 15),
    fg="white",
    bg="black",
    state=DISABLED,
    command=lambda: show_details(student_personal_homepage),
)
...
bg_bdy = "#90caf9"
# Name
Label(master=tab1, text="Full Name", font=("Comic Sans MS", 13), bg=bg_bdy).place(
    x=10, y=10
)
ent_name_get = Entry(
    master=tab1,
...
# Sex
Label(master=tab1, text="Sex", font=("Comic Sans MS", 13), bg=bg_bdy).place(
    x=10, y=80, height=30
)
...
# Category
Label(master=tab1, text="Category", font=("Comic Sans MS", 13), bg=bg_bdy).place(
    x=10, y=120
)
...
# Religion
Label(master=tab1, text="Religion", font=("Comic Sans MS", 13), bg=bg_bdy).place(
    x=10, y=160
)
lbl_religion_get = Label(
    tab1, text="", font=("times", 13), bg="white", bd=1, relief=SOLID
)
lbl_religion_get.place(x=210, y=160, height=30, width=150)


# Nationality
Label(master=tab1, text="Nationality", font=("Comic Sans MS", 13), bg=bg_bdy).place(
    x=10, y=200
)
lbl_nationality_get = Label(
    tab1, text="", font=("times", 13), bg="white", bd=1, relief=SOLID
)
...
# Physically Handicapped
Label(
    master=tab1, text="Physically Handicapped?", font=("Comic Sans MS", 13), bg=bg_bdy
).place(x=10, y=240)
lbl_physical_handicap_get = Label(
    tab1, text="", font=("times", 13), bg="white", bd=1, relief=SOLID
)
...
# % of Disability
Label(master=tab1, text="% of Disability", font=("Comic Sans MS", 13), bg=bg_bdy).place(
    x=10, y=280
)
lbl_percntg_hc_get = Label(
    tab1, text="", font=("times", 13), bg="white", bd=1, relief=SOLID
)
...
# 	Date of Birth
Label(master=tab1, text="Date of Birth", font=("Comic Sans MS", 13), bg=bg_bdy).place(
    x=10, y=320
)
...
# Country Name
Label(master=tab1, text="Country", font=("Comic Sans MS", 13), bg=bg_bdy).place(
    x=10, y=360
)
lbl_country_get = Label(
    tab1, text="", font=("times", 13), bg="white", bd=1, relief=SOLID
)
lbl_country_get.place(x=150, y=360, height=30, width=210)

# Aadhaar No.
Label(master=tab1, text="Aadhaar Number", font=("Comic Sans MS", 13), bg=bg_bdy).place(
    x=10, y=400
)

ent_aadhaar_get = Entry(
    master=tab1,
    bg="white",
    font=("times", 13),
    bd=1,
...
# ----------------------1st tab 2nd column---------------------------------------------
# Father's Name
Label(master=tab1, text="Father's Name", font=("Comic Sans MS", 13), bg=bg_bdy).place(
    x=400, y=10
)

ent_father_name_get = Entry(
    master=tab1,
    bg="white",
    font=("times", 13),
    bd=1,
...
# 	Father's Occupation
Label(
    master=tab1, text="Father's Occupation", font=("Comic Sans MS", 13), bg=bg_bdy
).place(x=400, y=80)

ent_father_occu_get = Entry(
    master=tab1,
    bg="white",
    font=("times", 13),
    bd=1,
...
# Mother's Name
Label(master=tab1, text="Mother's Name", font=("Comic Sans MS", 13), bg=bg_bdy).place(
    x=400, y=150
)

ent_mother_name_get = Entry(
    master=tab1,
    bg="white",
    font=("times", 13),
    bd=1,
...
# 	Mother's Occupation
Label(
    master=tab1, text="Mother's Occupation", font=("Comic Sans MS", 13), bg=bg_bdy
).place(x=400, y=220)

ent_mother_occu_get = Entry(
    master=tab1,
    bg="white",
    font=("times", 13),
    bd=1,
...
# Guardian Name
Label(master=tab1, text="Guardian Name", font=("Comic Sans MS", 13), bg=bg_bdy).place(
    x=400, y=290
)

ent_guardian_get = Entry(
    master=tab1,
    bg="white",
    font=("times", 13),
    bd=1,
...
# Guardian Relation
Label(
    master=tab1, text="Relation with Guardian", font=("Comic Sans MS", 13), bg=bg_bdy
).place(x=400, y=360)

ent_guardian_rel_get = Entry(
    master=tab1,
    bg="white",
    font=("times", 13),
    bd=1,
...
# Guardian Email Address
Label(
    master=tab1, text="Guardian's Email Address", font=("Comic Sans MS", 13), bg=bg_bdy
).place(x=400, y=430)

ent_g_email_get = Entry(
    master=tab1,
    bg="white",
    font=("times", 13),
    bd=1,
...
# Session
Label(master=tab1, text="Session", font=("Comic Sans MS", 13), bg=bg_bdy).place(
    x=760, y=10
)
lbl_session_get = Label(
    tab1, text="", font=("times", 13), bg="white", bd=1, relief=SOLID
)
lbl_session_get.place(x=860, y=10, height=30, width=120)

# Registration number
Label(master=tab1, text="Reg. No.", font=("Comic Sans MS", 13), bg=bg_bdy).place(
    x=760, y=50
)

ent_registration_get = Entry(
    master=tab1,
    bg="white",
    font=("times", 13),
    bd=1,
...
# Student Id
Label(master=tab1, text="Student Id", font=("Comic Sans MS", 13), bg=bg_bdy).place(
    x=760, y=120
)

ent_student_id_get = Entry(
    master=tab1,
    bg="white",
    font=("times", 13),
    bd=1,
...
# Course
Label(master=tab1, text="Course", font=("Comic Sans MS", 13), bg=bg_bdy).place(
    x=760, y=190
)
lbl_get_course = Label(
    tab1, text="", font=("times", 13), bg="white", bd=1, relief=SOLID
)
lbl_get_course.place(x=860, y=190, height=30, width=120)

# Phone Number
Label(master=tab1, text="Contact Number", font=("Comic Sans MS", 13), bg=bg_bdy).place(
    x=760, y=230
)
...
# Email Address
Label(master=tab1, text="Email Address", font=("Comic Sans MS", 13), bg=bg_bdy).place(
    x=760, y=300
)
...
# Guardian Phone Number
Label(
    master=tab1, text="Guardian's Contact Number", font=("Comic Sans MS", 13), bg=bg_bdy
).place(x=760, y=370)

ent_g_phone_get = Entry(
    master=tab1,
    bg="white",
    font=("times", 13),
    bd=1,
...
# Button to datatable
btn_home_tab1 = Button(
    tab1,
    text="Back to Select Student",
    font=("times", 15),
    fg="white",
    bg="green",
    activebackground="green",
    activeforeground="white",
    command=lambda: swap(frame_admin_panel),
)
...
# Permanent Address
Label(
    master=tab2, text="Permanent Address", font=("Comic Sans MS", 13), bg=bg_bdy
).place(x=10, y=10)

text_prmnt_add_get = Text(
    master=tab2,
    bg="white",
    font=("times", 12),
    bd=1,
...
# Pin Code
Label(master=tab2, text="Pincode", font=("Comic Sans MS", 13), bg=bg_bdy).place(
    x=10, y=150
)

ent_pincode_p_get = Entry(
    master=tab2,
    bg="white",
    font=("times", 13),
    bd=1,
...
# Present Address
Label(master=tab2, text="Present Address", font=("Comic Sans MS", 13), bg=bg_bdy).place(
    x=10, y=220
)

text_prsnt_add_get = Text(
    master=tab2,
    bg="white",
    font=("times", 12),
    bd=1,
...
# --------------------2nd Tab 2nd column---------------------------
# City name
Label(master=tab2, text="City", font=("Comic Sans MS", 13), bg=bg_bdy).place(
    x=360, y=10
)
...
# District Name
Label(master=tab2, text="District", font=("Comic Sans MS", 13), bg=bg_bdy).place(
    x=360, y=80
)

ent_district_get = Entry(
    master=tab2,
    bg="white",
    font=("times", 13),
    bd=1,
...
# State Name
Label(master=tab2, text="State", font=("Comic Sans MS", 13), bg=bg_bdy).place(
    x=360, y=150
)
...
# Last Institution
Label(
    master=tab2, text="Last Institution Name", font=("Comic Sans MS", 13), bg=bg_bdy
).place(x=700, y=10)

ent_last_instn_get = Entry(
    master=tab2,
    bg="white",
    font=("times", 13),
    bd=1,
...
# Last Institution Address
Label(
    master=tab2, text="Last Institution Address", font=("Comic Sans MS", 13), bg=bg_bdy
).place(x=700, y=80)

txt_last_instn_addrs_get = Text(
    master=tab2, bg="white", font=("times", 12), bd=1, relief=SOLID, cursor="arrow"
)
txt_last_instn_addrs_get.place(x=700, y=110, width=360, height=60)


# Hobbies
Label(master=tab2, text="Hobbies *", font=("Comic Sans MS", 13), bg=bg_bdy).place(
    x=700, y=180
)

txt_hobby_get = Text(
    master=tab2, font=("times", 13), bg="white", bd=1, relief=SOLID, cursor="arrow"
)
txt_hobby_get.place(x=700, y=210, width=360, height=60)


# Frame for marks
frame_marks_get = Frame(
    tab2,
    bg="lightgray",
    bd=1,
    relief=SOLID,
...
# Serial Number
Label(
    frame_marks_get, text="Sl.\nNo.", font=("times", 13, "bold"), bd=1, relief=RAISED
).place(x=0, y=0, width=40, height=50)

# Exam name
Label(
    frame_marks_get, text="Exam Name", font=("times", 13, "bold"), bd=1, relief=RAISED
).place(x=40, y=0, width=170, height=50)

# Board name
Label(
    frame_marks_get, text="Board Name", font=("times", 13, "bold"), bd=1, relief=RAISED
).place(x=210, y=0, width=200, height=50)
...
# Percentage
Label(frame_marks_get, text="%", font=("times", 13, "bold"), bd=1, relief=RAISED).place(
    x=550, y=0, width=70, height=50
)

# CGPA
Label(
    frame_marks_get, text="CGPA", font=("times", 13, "bold"), bd=1, relief=RAISED
).place(x=620, y=0, width=99, height=50)

# -----------------------------------Data--------------------------------------------

# Serial Numbers
Label(frame_marks_get, text="1", font=("times", 13, "bold"), bd=1, relief=SOLID).place(
    x=-1, y=49, height=30, width=41
)
Label(frame_marks_get, text="2", font=("times", 13, "bold"), bd=1, relief=SOLID).place(
    x=-1, y=78, height=30, width=41
)
Label(frame_marks_get, text="3", font=("times", 13, "bold"), bd=1, relief=SOLID).place(
    x=-1, y=107, height=30, width=41
)
Label(frame_marks_get, text="4", font=("times", 13, "bold"), bd=1, relief=SOLID).place(
    x=-1, y=136, height=30, width=41
)
